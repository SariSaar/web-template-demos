...
export const transitions = {
  ...
  // The off-session payment is initiated by the automatic transition
  // that marks the transaction ready for payment
  MARK_PENDING_PAYMENT: 'transition/mark-pending-payment',

  // If the customer's payment method is valid and has enough funds,
  // the payment is processed automatically in the Flex backend
  AUTO_PAYMENT: 'transition/auto-payment',

  // If the payment cannot be processed automatically, the customer needs
  // to initiate the payment manually.

  INITIATE_PAYMENT: 'transition/initiate-manual-payment',

    // Stripe SDK might need to ask 3D security from customer, in a separate front-end step.
  // Therefore we need to make another transition to Marketplace API,
  // to tell that the payment is confirmed.
  CONFIRM_PAYMENT: 'transition/confirm-manual-payment',

  // If the payment has been initiated but not completed, the operator can cancel the transaction
  // from the pending-confirmation state.
...
export const graph = {
  ...
  states: {
    ...
    [states.INQUIRY]: {
      on: {
        [transitions.REQUEST_BOOKING_AFTER_INQUIRY]: states.PENDING,
      },
    },
    ...
    [states.PENDING_CONFIRMATION]: {
      on: {
        ...
        [transitions.CANCEL_FROM_PENDING_CONFIRMATION]: states.CANCELED
      }
    },
...
export const isPrivileged = transition => {
  return [transitions.REQUEST_BOOKING, transitions.REQUEST_BOOKING_AFTER_INQUIRY].includes(
    transition
  );
};
...
export const statesNeedingCustomerAttention = [states.PENDING_PAYMENT];